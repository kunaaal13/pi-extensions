...
// Pi has no public transcript/tool-grouping hook (checked against
// @earendil-works/pi-coding-agent 0.84.1). Grouping adjacent tool rows into
// one block means patching two prototypes: ToolExecutionComponent (per-row
// presentation) and Container (which rows sit next to which). Every patch is
// guarded, idempotent, and restored on session_shutdown, so a Pi internals
// change makes this silently no-op instead of breaking the session.

const PRESENTATION_PATCHED = Symbol.for("kg.pi.toolGroups.presentation.v1");
const GROUPING_PATCHED = Symbol.for("kg.pi.toolGroups.grouping.v1");
...
// its own error-colored block; an expanded call keeps Pi's native detail.
// No same-message-vs-sequential distinction here (the upstream extension has
// one, gated behind an env var) — every contiguous run of collapsed calls
// merges, so behavior does not depend on how the calls were dispatched.
...
function installPresentationPatch(): PresentationPatchState | undefined {
  try {
    ...
    const patchedUpdateDisplay = function updateDisplayWithGrouping(
      ...
      try {
        if (!isLiveRow(this)) collapseSuccessfulResult(this);
      } catch {
